...
| **Ops Dashboard (first frontend + human SSO, de-risks CPQ)** | ✓ | ✓ | ✓ **26 tests, verified live incl. real Keycloak login** | ✓ |
...
├── src/ops-dashboard/  ★ REAL CODE — first frontend + human SSO, verified LIVE (26 tests, real Keycloak login)
...
- **`src/ops-dashboard/`** — read-only ops view over TMS/Rate/Masterdata;
  ...
  CPQ's harder approval-UI build. 26 tests, verified live through a full
  browser-equivalent login→dashboard→logout→re-login round trip.
- **213 tests total, all green**, across `rfq_common` (57) · `mock-fx` (37) ·
  `mock-masterdata` (24) · `mock-tms` (37) · `mock-rate` (25) ·
  `policy-evaluation` (7) · `ops-dashboard` (26).
