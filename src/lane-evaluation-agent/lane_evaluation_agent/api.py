"""lane-evaluation-agent -- M5a's A2A server. Agent Card advertises ONE
business-level skill (evaluate-lane-capacity), not the MCP tool
(check_lane_capacity) it happens to use internally -- the remote caller
asks for a business outcome, this agent decides how to satisfy it."""
...
from .executor import SKILL_ID, LaneCapacityExecutor
...
def _agent_card(public_url: str) -> AgentCard:
    return AgentCard(
        ...
        description="Evaluate whether a proposed logistics lane has capacity. "
                    "Not a general-purpose agent -- one narrow, real skill.",
        version="0.1.0",
        ...
        skills=[
            AgentSkill(
                id=SKILL_ID,
                name="Evaluate lane capacity",
                description="Evaluate whether a proposed logistics lane has capacity.",
                tags=["logistics", "capacity"],
...
def build_app(
    *,
    ...
    tms_mcp_client: httpx.AsyncClient | None = None,
    ...
    tms_mcp_url = settings.tms_mcp_url()
    tms_mcp_client = tms_mcp_client or httpx.AsyncClient(base_url=tms_mcp_url, timeout=5.0)
    ...
    app = create_app("Lane Evaluation Agent (A2A)", system_id="lane-evaluation-agent")
    add_a2a_routes_to_fastapi(
        app,
    ...
    def descriptor() -> dict:
        return build_descriptor(
            ...
            dependencies=[DependencyInfo(
                canonical_id="workload.tms-mcp", relation="mcp", endpoint=tms_mcp_url,
            )],
        ).model_dump()
