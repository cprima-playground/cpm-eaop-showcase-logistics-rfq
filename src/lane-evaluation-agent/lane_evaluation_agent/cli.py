...
def main() -> None:
    ...
    identity = MachineIdentity(
        ...
        client_id=settings.KEYCLOAK_CLIENT_ID, secret_name="lane-evaluation-agent-client-secret",
    )
    asyncio.run(run_startup_self_check(identity, keycloak_url=settings.keycloak_url(), root=RFQ_ROOT))

    instance_id = new_instance_id()
    configure_observability(
        service_name="lane-evaluation-agent", canonical_id=settings.EXPECTED_CANONICAL_ID,
    ...
    port = int(os.environ.get("SERVICE_PORT", os.environ.get("A2A_PORT", "8204")))
