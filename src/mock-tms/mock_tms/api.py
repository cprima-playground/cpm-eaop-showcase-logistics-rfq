...
from rfq_common.models import RouteAvailability, RouteAvailabilityStatus
from rfq_common.secrets import SecretsClient
from rfq_common.theme import load_theme

from .auth import require_api_key
from .store import TmsStore, UnknownLocationError
...
def build_app(*, fixtures_dir: Path | None = None, masterdata_client: MasterdataClient | None = None) -> FastAPI:
    ...
    def list_routes() -> list[dict]:
        return [r.model_dump(mode="json", by_alias=True) for r in store.list_routes()]
    ...
    def feasible_lanes(lane: str) -> list[dict]:
        return [r.model_dump(mode="json", by_alias=True) for r in store.feasible_lanes(lane)]
