...
def main() -> None:
    ...
    identity = MachineIdentity(
        ...
        client_id=settings.KEYCLOAK_CLIENT_ID, secret_name="route-decision-agent-client-secret",
    )
    asyncio.run(run_startup_self_check(identity, oidc_issuer_url=settings.oidc_issuer_url(), root=RFQ_ROOT))

    instance_id = new_instance_id()
    configure_observability(
        service_name="route-decision-agent", canonical_id=settings.EXPECTED_CANONICAL_ID,
    ...
    port = int(os.environ.get("SERVICE_PORT", os.environ.get("A2A_PORT", "8205")))
