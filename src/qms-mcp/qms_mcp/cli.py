...
def _resolve_host_port() -> tuple[str, int]:
    ...
    port = int(os.environ.get("SERVICE_PORT", os.environ.get("A2A_PORT", "8106")))
    return host, port
...
def main() -> None:
    ...
    identity = MachineIdentity(
        ...
        client_id=settings.KEYCLOAK_CLIENT_ID, secret_name="qms-mcp-svc-client-secret",
    )
    asyncio.run(run_startup_self_check(identity, keycloak_url=settings.keycloak_url(), root=RFQ_ROOT))

    instance_id = new_instance_id()
    configure_observability(
        service_name="qms-mcp", canonical_id=settings.EXPECTED_CANONICAL_ID,
