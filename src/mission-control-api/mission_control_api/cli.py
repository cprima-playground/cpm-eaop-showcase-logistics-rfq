...
def _resolve_host_port() -> tuple[str, int]:
    ...
    port = int(os.environ.get("SERVICE_PORT", os.environ.get("A2A_PORT", "8300")))
    return host, port
...
def main() -> None:
    ...
    identity = MachineIdentity(
        ...
        client_id=settings.KEYCLOAK_CLIENT_ID, secret_name="mission-control-api-svc-client-secret",
    )
    asyncio.run(run_startup_self_check(identity, oidc_issuer_url=settings.oidc_issuer_url(), root=RFQ_ROOT))

    instance_id = new_instance_id()
    configure_observability(
        service_name="mission-control-api", canonical_id=settings.EXPECTED_CANONICAL_ID,
