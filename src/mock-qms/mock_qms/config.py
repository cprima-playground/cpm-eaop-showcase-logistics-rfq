"""Env + secrets resolution for the QMS frontend -- same fail-closed pattern
...
Mirrors ops_dashboard/config.py -- same shape, QMS's own client/secrets."""
...
def keycloak_client_id() -> str:
    return os.environ.get("QMS_CLIENT_ID", "qms-web")


def public_base_url() -> str:
    return os.environ.get("QMS_PUBLIC_URL", "http://localhost:8007")
...
def caddy_ssl_context() -> ssl.SSLContext | bool:
    ...
    normal default) when there's no local CA to trust (direct :8081/:8007
...
def keycloak_client_secret() -> str:
    ...
    env_value = os.environ.get("QMS_CLIENT_SECRET")
    if env_value:
    ...
    try:
        _cached_client_secret = SecretsClient("dev", inventory_path=INVENTORY_PATH).get(
            "qms-web-client-secret"
        )
        return _cached_client_secret
    except Exception as exc:
        raise CredentialUnavailableError(
            "qms-web-client-secret is not available: QMS_CLIENT_SECRET "
            ...
            "`terraform output qms_web_client_secret` into Vault -- see "
...
def session_secret() -> str:
    ...
    env_value = os.environ.get("QMS_SESSION_SECRET")
    if env_value:
    ...
    try:
        ...
        raise CredentialUnavailableError(
            "qms-session-secret is not available: QMS_SESSION_SECRET "
            "is unset and Vault could not supply it (docker compose up -d in "
