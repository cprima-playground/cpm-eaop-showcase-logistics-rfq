"""Human SSO login -- Authorization Code + PKCE against the dev Keycloak
realm (infra/keycloak/), own client (qms-web). Mirrors ops_dashboard/sso.py
verbatim: claims come from verifying the access token via
rfq_common.verify.verify_with_jwks() (signature/issuer/audience checked)
rather than trusting /userinfo unverified, then resolve_principal() turns
claims into a typed Principal. Post-login lands on /app (QMS's UI root),
not "/" -- that's the JSON API's own search route (GET /quotes's sibling),
not a page.
"""
...
def callback(request: Request, code: str, state: str):
    if state != request.session.get("oauth_state"):
        return RedirectResponse("/app?error=state_mismatch")
